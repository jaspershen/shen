...
# )
# if(all(is.na(gh_data$count))){
#   number <- sample(c(0,1), nrow(gh_data), replace = TRUE)
# gh_data$count[is.na(gh_data$count)] <- number
# gh_data$tip <- paste(number, "contributions", sep = " ")
# }
...
# 
# talk_data <-
#   purrr::map_dfr(set_names(talks, dirname(talks)), function(file) {
#     data <-
#       rmarkdown::yaml_front_matter(here::here("content/en/talk", file))
#     list(
#       date = as.Date(data$date),
#       title = data$title,
...
# p_data <-
#   dplyr::filter(all_data, date >= last_year, date <= today()) %>%
#   dplyr::mutate(
...
#   )
# 
# if(!"post" %in% unique(p_data$type)){
#   temp <- 
#     p_data %>% 
#     dplyr::filter(type == "github") %>% 
#     head(1) %>% 
...
# 
# counts <- 
#   dplyr::count(p_data, type, .drop = FALSE) %>%
#   na.omit() %>%
#   dplyr::filter(type %in% c("github", "post")) %>% 
#   dplyr::mutate(
#     n = case_when(
...
#     axis.text = element_text(color = "#991500", size = 8),
#     axis.title = element_text(color = "#991500"),
#     axis.ticks = element_line(color = "#991500"),
#     panel.border = element_rect(color = "#991500")
#   ) +
#   guides(
#     alpha = "none",
#     fill = guide_legend(
...
#                           size = 1),
#       keywidth = 1,
#       label.theme = element_text(color = "#991500"),
...
[Google scholar link](https://scholar.google.co.uk/citations?hl=en&user=3TK9yz8AAAAJ)
...
ggplot(cit, aes(x = year, y = cites)) +
  geom_bar(stat = "identity", 
           ...
           fill = ggplot2::alpha("#991500", 0.5)) +
    xlab('Year')+
    ylab('Number of cites') +
    theme_void() +
  ggplot2::ggtitle(paste("Google Scholar total citations:", profile$total_cites)) +
